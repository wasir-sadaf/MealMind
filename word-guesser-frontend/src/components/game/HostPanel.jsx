...
const HostPanel = ({ gameId, questions, gameStatus }) => {
  ...
  const handleAnswer = async (answer) => {
    ...
    setLoading(true);
    try {
      // Use questionIndex instead of question_id
      const questionIndex = currentQuestion.index !== undefined 
        ? currentQuestion.index 
        : questions.indexOf(currentQuestion);
      await gameService.answerQuestion(gameId, questionIndex, answer);
    } catch (error) {
